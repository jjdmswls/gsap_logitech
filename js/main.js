window.addEventListener('DOMContentLoaded', function () {
  ...
  gsap.to('#section02 .we', {
    scrollTrigger: {
      ...
      start: 'top top', // 앞의 값 : trigger에 적용되어 있는 요소의 위치, 뒤의 값 : 화면의 표시 위치
      end: '109.2% bottom', // 앞의 값 : trigger에 적용되어 있는 요소의 위치, 뒤의 값 : 화면의 표시 위치
      scrub: true,
      pin: true, // trigger에 지정된 요소의 크기만큼 화면을 고정
      // markers: true,
    },
  ...
  //section03 blue box active when added class on
  gsap.to('#section03', {
    scrollTrigger: {
      trigger: '#section03',
      start: 'top top',
  ...
  sectionCount.forEach((section) => {
    sec05_toral += section.clientWidth; //각각의 page 가로 크기 합산
  });

  // console.log(sec05_toral);
  // console.log(innerWidth);

  gsap.to('.fix-this-5', {
    x: -(sec05_toral - innerWidth),
    scrollTrigger: {
  ...
  //page 3 text scroll up anumation
  gsap.to('#section05 .page03 .txt', {
    y: -innerHeight * 2,
    scrollTrigger: {
      trigger: '#section05',
      start: () => '+=' + this.innerHeight * 1.5, //화면 높이 길이의 하나 반 만큼 가로 스크롤이 이동했을 때 start 마커가 나타남
      scrub: true,
      //markers: true,
    },
  });

  //page 4 text anumation
  ...
  gsap.to('#section05', {
    scrollTrigger: {
      ...
      onLeaveBack: () => {
        video05.currentTime = 0; //비지오 재생 시간을 0으로 되돌림
        video05.pause();
      },
    },
  });
